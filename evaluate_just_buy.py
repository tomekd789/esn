"""
Run inference taking the "just buy" strategy at the selected ticker value
...
def parse_command_line_arguments():  # pylint: disable=missing-function-docstring
    ...
    parser.add_argument("--start_at", type=int, help="Sequence start point")
...
def main(args):  # pylint: disable=missing-function-docstring
    ...
    for sequence_counter in range(args.sequences):
        ...
        sequence = sequence[args.start_at:]
        ...
        cash += cash * gain
        trade_duration_weeks = (sequence_counter + 1) * len(sequence) / ONE_WEEK
